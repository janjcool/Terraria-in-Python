...
class ButtonController:
    ...
    def game(self, config_dict, UI_config_dict):
        ...
        if config_dict["variables"]["general"]["event_dict"].get('escape_down') == True:
            misc.menuManager(config_dict, UI_config_dict, "GameMenu")
    
    def mainMenu(self, config_dict, UI_config_dict):

        if config_dict["variables"]["general"]["event_dict"].get('start button') == True:
            misc.menuManager(config_dict, UI_config_dict, "Game")
        if config_dict["variables"]["general"]["event_dict"].get('exit') == True:
            misc.exit()
        if config_dict["variables"]["general"]["event_dict"].get('settings') == True:
            print("does nothing")
        
    def gameMenu(self, config_dict, UI_config_dict):
            
        if config_dict["variables"]["general"]["event_dict"].get('exit') == True:
            misc.exit()
            
        if config_dict["variables"]["general"]["event_dict"].get('MainMenu button') == True:
            misc.menuManager(config_dict, UI_config_dict, "MainMenu")
