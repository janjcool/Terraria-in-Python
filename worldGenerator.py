...
class world:
    def __init__(self, config_options, variables_class):
        self.config_options = config_options
        self.variables_class = variables_class
        self.world_class = entity.world(variables_class.map_height, variables_class.map_width, config_options)
        ...
        print(self.world_class.world)

    def mapHeight_maker(self):
        self.raw_mapHeight = []
        self.mapHeight = []
        noise = OpenSimplex(seed=self.variables_class.seed)

        #get raw noise data
        for x in range(0, int(self.variables_class.map_width/self.config_options.distance_between_raw+2)):
        ...
        for x in range(0, int(self.variables_class.map_width/self.config_options.distance_between_raw)):
            temp_value = 0
    ...
    def chunk_maker(self):
        self.world_class.world = self.world = numpy.empty( (int(self.variables_class.map_height/self.config_options.chunk_size), int(self.variables_class.map_width/self.config_options.chunk_size)), dtype=object)
        
        for x in range(0, int(self.variables_class.map_height/self.config_options.chunk_size)):
            for y in range(0, int(self.variables_class.map_width/self.config_options.chunk_size)):                
    ...
    def make_sheet_with_cordinats(self):
        ...
        for x in self.mapHeight:
            for y in range(0, self.config_options.amount_of_sky): #add sky
                #self.world[y, counter] = entity.block("sky")
            ...
            for i in range(0, x): #add more sky
                #self.world[y + i, counter] = entity.block("sky")
            ...
            for q in range(0, self.variables_class.map_height-x-self.config_options.amount_of_sky+2):
                #self.world[y + i + q, counter] = entity.block("dirt")
